...
段数は「レジスタ / BRAM ポート間の最長経路に並ぶ LUT・CARRY4・MUXF の数」です。

**CAM セル版（`CAM_IMPL = 0`、ASIC の本設計と同じ構造）**

| 構成（bit × エントリ × 次元） | CAM bit | LUT | FF | BRAM | 段数 | LUT / CAM bit |
|---|---|---|---|---|---|---|
| 32 × 256 × 1 | 8,192 | 13,457 | 8,490 | 1 | 16 | 1.64 |
| 32 × 256 × 2 | 16,384 | 15,413 | 16,682 | 1 | 17 | 0.94 |
| 32 × 1024 × 1 | 32,768 | 53,825 | 33,836 | 2 | 18 | 1.64 |

**BRAM 版（`CAM_IMPL = 1`、FPGA 向けの差し替え）**

| 構成 | CAM bit | LUT | FF | BRAM（RAMB36 換算） | 段数 |
|---|---|---|---|---|---|
| 32 × 256 × 1 | 8,192 | 3,201 | 458 | 17 | 12 |
| 32 × 256 × 2 | 16,384 | 5,496 | 804 | 33 | 12 |
| 32 × 1024 × 1 | 32,768 | 11,648 | 1,230 | 60 | 14 |

読み方:

- **CAM セル版は CAM 1 bit ≈ FF 1 個 + LUT 1〜1.6 個。** FF は記憶ビットそのもの（+ 行ごとの valid）。
  LUT は XNOR とマスク OR が LUT3 に畳まれた分と、行 AND・次元 AND・エンコーダーの木。
  LUT 数は abc の最適化結果で 1〜2 割ふれる（2 次元構成は行 AND を 1 本の木にまとめられる分だけ少ない）
- **BRAM 版は LUT が 1/4、FF が 1/20。** BRAM は 1 スライス（8 bit）あたり RAMB36 × 4（256 エントリ）で、
  4 スライス = 16 個 + データ SRAM 1 個。エントリ数に比例して増える
- **データ SRAM は BRAM に載る**（256 × 64 で RAMB36 1 個）。CAM 側の資源とは競合しない
- **段数は「検索 + SRAM 読み出し」を 1 クロックで通した長さ**（addr → CAM 比較 → 行 AND →
  エンコーダー → SRAM アドレス）。1 段 ≈ 0.5 ns（LUT + 配線）と見て、CAM セル版で 8〜9 ns、
  Artix-7 で **100 MHz 前後**が目安。BRAM 版は BRAM の同期読み出しで経路が切れる分だけ短い。
  500 MHz は ASIC（28 nm）の数字であって FPGA では出ない
- **Artix-7 での収まり:**
  - CAM セル版: XC7A35T（LUT 20,800）に 32 × 256 × 1、XC7A100T（63,400）に 32 × 256 × 2。
    32 × 1024 × 1 は XC7A100T で 85% と窮屈で XC7A200T（134,600）向け
  - BRAM 版: XC7A35T（RAMB36 50 個）に 32 × 256 × 2、XC7A100T（135 個）に 32 × 1024 × 1 が
    LUT 18% / BRAM 44% で余裕をもって載る

クロックを上げたいときは、エンコーダーの出力（`hit_index`）にレジスタを 1 段入れて
検索と SRAM 読み出しを分ける。Read が 1 クロック増え、経路は半分になる。

### BRAM 版 CAM の構造 / BRAM-based CAM

FPGA で 1024 エントリ以上を試すには、CAM セル版（FF + LUT）は資源を食いすぎます。
`rtl/cam_array_bram.v` は同じインターフェースの差し替え実装で、比較器を並べる代わりに
**表を引き**ます。これは FPGA 専用の技法で、ASIC では上の CAM セル構造が本設計です。

**スライスごとの一致表** — アドレスを `SLICE_BITS`（8）ビットずつのスライスに切り、
スライスごとに BRAM を持ちます。行アドレス = スライス値、データ = エントリごとの一致ビット列。

```
input_addr[31:24] → BRAM#3 → vec3 ┐
input_addr[23:16] → BRAM#2 → vec2 ├─ row_hit[i] = vec3[i] & vec2[i] & vec1[i] & vec0[i] & valid[i]
input_addr[15: 8] → BRAM#1 → vec1 │
input_addr[ 7: 0] → BRAM#0 → vec0 ┘       → and_reduction_tree / priority_encoder はそのまま
```

**LSB マスクはプレフィックス表で引く** — マスクは検索のたびに変わるので、普通の BRAM-CAM では
「下位 m ビットが何でもよい 2^m 行を全部読む」ことになり遅すぎます。空間メモリーは
**LSB から順に剥がす**という規則があるので、段 j（下位 j ビット無視）ごとの表を並べておけば、
どの段でも読み出しは 1 回です。

```
行アドレス = { j, v >> j }     j = 0 .. SLICE_BITS-1
  j=0 : 256 行（完全一致）
  j=1 : 128 行（下位 1 bit 無視）
  ...
  j=7 :   2 行
  合計 510 行 → 512 行の BRAM にちょうど収まる（表は 2 倍弱）
```

検索時はグローバルマスクからスライスごとに「剥がされたビット数 m_s」を求め、
`m_s == SLICE_BITS` なら読まずに全 1（バイパス）、それ以外は行 `{ m_s, v_s >> m_s }` を読みます。
Morton コードで D ビットずつ剥がす場合も m_s が D 刻みになるだけです。

**登録** — エントリ i のアドレスが決まると、bit i を立てるべき行はスライスごと・段ごとに
ちょうど 1 行です。各スライスを並列に「読む → bit i を立てる → 書き戻す」を段の数だけ繰り返し、
**2 × SLICE_BITS = 16 クロック**。その間 `busy` を出します。valid は全段そろってから立てます。
削除を足すなら同じ行を辿って bit i を落とすだけで、同じクロック数で済みます。

**ポートの使い方** — 読み出しはポート A だけ（検索も、登録中の読み出しも）、ポート B は書き込み専用。
両ポートで読むと BRAM が TDP モードになりポート幅が 36 bit に落ちて個数が倍になります
（実測で RAMB36 61 個 → 17 個）。busy 中の検索はどのみち保証しないので、登録中はポート A を
登録側に貸します。

**リセット** — BRAM の中身は不定なので、リセット解除後に全行を 0 で埋めます
（2^(SLICE_BITS+1) = 512 クロック、`busy`）。CAM セル版には無い手順です。

**遅延** — BRAM の読み出しは同期なので `match_vec` は 1 クロック遅れます。トップ
（`top_spatial_memory`）は `CAM_IMPL` に応じて要求を 1 段パイプラインし、`resp_valid` で
結果の有効なクロックを示します。CAM セル版では `resp_valid` は要求と同じクロックです。
経路が短くなるので、毎クロック 1 検索を受け付けながら CAM セル版より高いクロックで回せます。
近傍検索は段 0, 1, 2, … を連続するクロックで投機的に流し込めば、K 段が `K + 4` クロック程度で終わります。
