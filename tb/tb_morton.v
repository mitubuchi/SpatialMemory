...
`timescale 1ns/1ps
module tb_morton;
  parameter CAM_IMPL = 0;   // 0: CAM セル版 / 1: BRAM 版（iverilog -P tb_morton.CAM_IMPL=1）
  ...
  top_spatial_memory #(.BIT_WIDTH(BW), .ENTRY_BITS(EB), .NUM_ARRAYS(1), .DATA_WIDTH(DW), .SHIFT_STEP(2), .CAM_IMPL(CAM_IMPL)) dut (
