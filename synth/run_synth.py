...
FPGA の実配線遅延は入っていない。LUT depth はレジスタ間の最長経路に並ぶセル数
...
def synth(cfg, data_width, yosys):
...
chparam -set BIT_WIDTH {bw} -set ENTRY_BITS {eb} -set NUM_ARRAYS {na} -set DATA_WIDTH {data_width} top_spatial_memory
...
# FF セルを消してレジスタ間の組み合わせ経路だけを残す（stat の後なので資源量には影響しない）
delete t:FD* t:BUFG
...
def main():
    ...
    for cfg in args.config or DEFAULT_CONFIGS:
        ...
        r = synth(cfg, args.data, yosys)
        if r:
            rows.append(r)
    if not rows:
        return 1
    print()
    print(f"{'config':>12} {'CAM bits':>9} {'LUT':>7} {'FF':>7} {'BRAM':>5} {'DRAM':>5} {'CARRY4':>6} {'MUXF':>5} {'LUT depth':>9} {'LUT/bit':>8}")
    for r in rows:
        per = r["LUT"] / r["cam_bits"] if r["cam_bits"] else 0
        print(f"{r['config']:>12} {r['cam_bits']:>9} {r['LUT']:>7} {r['FF']:>7} {r['BRAM']:>5} {r['DRAM']:>5} "
